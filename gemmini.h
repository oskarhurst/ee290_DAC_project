...
static void sp_tiled_matmul_ws(const elem_t * A, const elem_t * B,
  ...
  const uint32_t A_sp_addr_start = 0;
  const uint32_t B_sp_addr_start = BANK_ROWS * BANK_NUM - I * K * DIM; //shift the starting address so B_sp_addr starts in a new bank 
  const uint32_t D_sp_addr_start = 1 << (ADDR_LEN-1);
  const uint32_t C_sp_addr_start = 3 << (ADDR_LEN-2) | (full_C << (ADDR_LEN-3));
...
#ifdef HAS_MVIN_ACC_SCALE
#define GEMMINI_ACC_SCALE(x, scale) MVIN_ACC_SCALE((x), (scale))
#else
  ...
  if (DIM_I % 4 == 0 && DIM_J % 4 == 0) {
    ...
    for (size_t i = 0; i < DIM_I; i++) {
      for (size_t j = 0; j < DIM_J; j++) {
        ...
        acc_t result = no_bias ? 0 : GEMMINI_ACC_SCALE(*(D + bias_row * stride_D + j), D_scale_factor);
...
void tiled_matmul_auto(size_t dim_I, size_t dim_J, size_t dim_K,
    ...
    //The maximum tile factors for j and i 
    const size_t max_f = ((size_t)sqrt(ACC_ROWS / DIM));

    //Set the tile factor to the dim_padded/DIM if this value is smaller than the maximum tile factor given our scratchpad
    tile_I = dim_I_padded/DIM < max_f ? dim_I_padded/DIM : max_f;
    tile_J = dim_J_padded/DIM < max_f ? dim_J_padded/DIM : max_f;
    tile_K = dim_K_padded/DIM < (BANK_ROWS * BANK_NUM / 2 / DIM / max_f) ? dim_K_padded/DIM : (BANK_ROWS * BANK_NUM / 2 / DIM / max_f);
